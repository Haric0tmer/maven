...
public class BankAccount {
	...
	// // Il y a un point de décision à chaque opérateur && en plus du if
	// if (withdrawAmount >= 0 && balance >= withdrawAmount && withdrawAmount <
	// withdrawLimit
	...
	public boolean withdrawMoney(double withdrawAmount) {
		if (isWithdrawalAllowed(withdrawAmount)) {
	...
	private boolean isWithdrawalAllowed(double amount) {
		...
		boolean sufficientBalance = balance >= amount;
		boolean underTransactionLimit = amount < withdrawLimit;
		boolean underCumulativeLimit = (amount + amountWithdrawn) <= withdrawLimit;

		return positiveAmount && sufficientBalance && underTransactionLimit && underCumulativeLimit;
	}
